...
if((isset($_POST['dtsearch']) ? $_POST['dtsearch'] : null) && (isset($_POST['fr_date'])) && (isset($_POST['castype'])) && (isset($_POST['vendor_nm']))){
	
	$from_dt = $_POST['fr_date'];
	$to_dt = $_POST['to_date'];
	$param_string=null;
	$connpdo_trclasic_dc = conn_treasraj_pdo();
	$sth = $connpdo_trclasic_dc->query("SELECT * from treasurystatus_control.cash_office_support_details WHERE report_date >= '$from_dt' AND report_date <= '$to_dt'");
	$rows = $sth->fetchAll();
	...
	$param_string=null;
	$connpdo_trclasic_dc = conn_treasraj_pdo();
	$sth = $connpdo_trclasic_dc->query("select * from treasurystatus_control.cash_office_support_details order by report_date DESC LIMIT 50");
	$rows = $sth->fetchAll();
	$connpdo_trclasic_dc = null;
	$sth->execute();
}
	...
	$param_string=null;
	$connpdo_trclasic_dc = conn_treasraj_pdo();
	$sth = $connpdo_trclasic_dc->query("select * from treasurystatus_control.cash_office_support_details WHERE report_date >= '$report_pos_dt' order by report_date");
			...
			<tr>
				<td>
					...
					<select name="castype" id="castype">
					  ...
					  <option value="treasurystatus_control">treasurystatus_control</option>
